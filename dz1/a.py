...
ровно одним пробелом (–50 ≤ t_room ≤ 50, –50 ≤ t_cond ≤ 50).
...
def get_t(t_room: int, t_cond: int, mode: str) -> int:
    if mode == "freeze":
...
if __name__ == "__main__":
    t_room, t_cond = [int(x) for x in input().split()]
    mode = input()
    res = get_t(t_room, t_cond, mode)
    print(res)
