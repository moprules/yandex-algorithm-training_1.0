...
def work(nums: list):
    ...
    if mp1 is not None and mp2 is not None:
        variants.append((mp1, mp2))
    if mm1 is not None and mm2 is not None:
        variants.append((mm1, mm2))
    # Если нет наборов чисто положительные(+0) и чисто отрицательные числа
    if not variants:
        # Значит смгли найти только два числа:
        # максимально отрицательное и максимальное положительное (+0)
        return mm1, mp2
    else:
        if len(variants) == 1:
            return variants[0]
